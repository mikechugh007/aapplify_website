...
class RequestDemoView(View):
    ...
    def get_context(self, form=None, success_message=None):
        return {
            'title': 'Book a Demo',
            'meta_title': 'AAPPLIFY | Book an Enterprise Control Plane Demo',
            'meta_description': 'Map one identity, change, data, or security workflow and see how AAPPLIFY connects capabilities, signals, orchestration, controls, and departmental consoles.',
    ...
    def post(self, request):
        ...
        message_body = (
            "New AAPPLIFY control plane demo request\n\n"
            f"Name: {cleaned['name']}\n"
        ...
        success_message = (
            ...
            "best path for a focused control-plane pilot."
        )
...
class ProductGuardView(View):
    def get(self, request):
        return render(request, 'pages/products/guard.html', {'title': 'AAPPLIFY Guard'})
...
class HomeView(View):
    def get(self, request):
        return render(request, 'pages/home.html', {
            ...
            'meta_title': 'AAPPLIFY | Enterprise Control Plane for Identity, Change, Data, and Security',
            'meta_description': 'AAPPLIFY turns identity, change, security, and policy data into governed action with departmental consoles, capabilities, signals, orchestration, controls, and modules.',
        })

class ReleaseReadinessLandingView(View):
    def get(self, request):
        return render(request, 'pages/release_readiness_landing.html', {
            'title': 'Change Control Use Case',
            'meta_title': 'AAPPLIFY | Change Control and Release Readiness Use Case',
            'meta_description': 'Release readiness is the change-control view of the AAPPLIFY control plane, connecting release, identity, data, security, and policy context into governed action.',
        })
    ...
    cards = [
        {
            ...
            'copy': 'Approve, hold, or investigate production change.',
            'href': reverse('landing_release_readiness'),
        },
        {
            'name': 'Identity',
            'title': 'Access risk',
            'copy': 'Review privilege drift and risky grants.',
            'href': reverse('landing_identity'),
        },
        {
            'name': 'Data',
            'title': 'Policy context',
            'copy': 'Map sensitive data movement to controls.',
            'href': reverse('landing_data'),
        },
        {
            'name': 'Security',
            'title': 'Exposure paths',
            'copy': 'Connect findings to remediation workflows.',
...
class IdentityLandingView(View):
    def get(self, request):
        return render(request, 'pages/domain_use_case.html', {
            'title': 'Identity Control Use Case',
            'meta_title': 'AAPPLIFY | Identity Access Risk Control Plane Use Case',
            'meta_description': 'Use AAPPLIFY to detect risky identity change, privilege drift, stale access, and missing controls, then trigger governed access review workflows.',
            'eyebrow': 'Identity control use case',
            'hero_title': 'Detect risky access change before it becomes exposure.',
            'hero_copy': 'Use this console when privileges, roles, service accounts, or emergency grants can create security risk. AAPPLIFY connects identity, change, data, security, and policy context into one governed access decision.',
            'placement_note': 'This is the identity view of the control plane. The same model extends across change, data, and security.',
            'console_label': 'Identity Console',
            'console_status': 'Review required',
            'console_title': 'Privileged Access Review',
            'console_copy': 'A new production role grant touches a sensitive data system and lacks owner approval evidence.',
            'metrics': [
            ...
            'fit_title': 'Identity risk is one domain console.',
            'fit_copy': 'Use it for access risk. Extend the same model across change, data, and security.',
            ...
            'signals': [
                {'kicker': 'Access drift', 'title': 'Privilege changed outside policy', 'copy': 'Detects role grants, group changes, or entitlement expansion that conflicts with policy or expected ownership.'},
                {'kicker': 'Service accounts', 'title': 'Non-human identity risk', 'copy': 'Surfaces stale owners, broad scopes, unused credentials, and service identities tied to critical systems.'},
                {'kicker': 'Sensitive reach', 'title': 'Access touches protected data', 'copy': 'Connects identity permissions to data classification, lineage, and control requirements.'},
            ],
            'workflow_title': 'Turn access signals into governed review.',
            'workflow_copy': 'Route identity risks to owners, require approvals, create remediation tasks, and preserve evidence for audits.',
            'workflow_steps': [
                {'number': 'Step 1', 'title': 'Connect', 'copy': 'Ingest IdP, PAM, HRIS, ticket, data, and cloud logs.'},
                {'number': 'Step 2', 'title': 'Signal', 'copy': 'Prioritize risky grants, stale owners, and policy conflicts.'},
            ...
            'cta_title': 'Build your first identity risk console.',
            'cta_copy': 'Map one access workflow and see how AAPPLIFY connects identity data, policies, signals, workflows, and controls.',
        })

class DataLandingView(View):
    def get(self, request):
        return render(request, 'pages/domain_use_case.html', {
            'title': 'Data Control Use Case',
            'meta_title': 'AAPPLIFY | Data Policy and Control Plane Use Case',
            'meta_description': 'Use AAPPLIFY to connect data movement, classification, policy files, ownership, and controls into governed data-risk workflows.',
            'eyebrow': 'Data control use case',
            'hero_title': 'Turn sensitive data movement into policy-aware action.',
            'hero_copy': 'Use this console when data classification, access, retention, or movement creates security and compliance risk. AAPPLIFY connects data, identity, change, security, and policy context into one governed decision.',
            'placement_note': 'This is the data view of the control plane. The same model extends across identity, change, and security.',
            'console_label': 'Data Console',
            'console_status': 'Policy match',
            'console_title': 'Sensitive Data Movement Review',
            'console_copy': 'Customer records moved to a new analytics workflow with policy requirements and owner approval pending.',
            'metrics': [
            ...
            'fit_title': 'Data risk is one domain console.',
            'fit_copy': 'Use it for data policy context. Extend the same model across identity, change, and security.',
            ...
            'signals': [
                {'kicker': 'Classification', 'title': 'Sensitive data enters a new workflow', 'copy': 'Detects when protected data moves into a system, report, pipeline, or app workflow that needs control review.'},
                ...
                {'kicker': 'Access + lineage', 'title': 'Data reach changes after access or system change', 'copy': 'Combines identity and lineage context so teams can see who can touch sensitive data and why.'},
            ],
            'workflow_title': 'Turn data signals into governed handling.',
            'workflow_copy': 'Route data risks to owners, validate controls, require approvals, and preserve policy evidence.',
            'workflow_steps': [
            ...
            'cta_title': 'Build your first data policy console.',
            'cta_copy': 'Map one sensitive data workflow and see how AAPPLIFY turns policies and telemetry into governed action.',
        })

class SecurityLandingView(View):
    def get(self, request):
        return render(request, 'pages/domain_use_case.html', {
            'title': 'Security Control Use Case',
            'meta_title': 'AAPPLIFY | Security Exposure Control Plane Use Case',
            'meta_description': 'Use AAPPLIFY to connect security findings, identity, data, change, controls, and workflows into governed remediation.',
            'eyebrow': 'Security control use case',
            'hero_title': 'Connect exposure paths to the workflows that reduce risk.',
            'hero_copy': 'Use this console when findings, vulnerabilities, posture gaps, incidents, or risky changes need action. AAPPLIFY connects security, identity, data, change, and policy context into one governed remediation path.',
            'placement_note': 'This is the security view of the control plane. The same model extends across identity, data, and change.',
            'console_label': 'Security Console',
            'console_status': 'Escalate',
            'console_title': 'Exposure Path Review',
            'console_copy': 'A vulnerable service has elevated identity reach, sensitive data access, and an unapproved configuration change.',
            'metrics': [
            ...
            'fit_title': 'Security risk is one domain console.',
            'fit_copy': 'Use it for exposure paths. Extend the same model across identity, data, and change.',
            ...
            'signals': [
                {'kicker': 'Exposure path', 'title': 'Finding connects to sensitive systems', 'copy': 'Correlates vulnerabilities and posture gaps with identity access, data reach, and asset ownership.'},
            ...
            'workflow_title': 'Turn security signals into governed remediation.',
            'workflow_copy': 'Route exposure paths to owners, require approvals, track remediation, and preserve evidence for security and compliance review.',
            'workflow_steps': [
            ...
            'cta_title': 'Build your first security exposure console.',
...
class AapplifyLandingView(View):
    def get(self, request):
        return render(request, 'pages/aapplify_landing.html', {
            'title': 'AAPPLIFY',
            'meta_title': 'AAPPLIFY | Control Plane for Security, Identity, Data, and Change',
            'meta_description': 'AAPPLIFY turns identity, change, security, and policy data into governed action with consoles, capabilities, signals, orchestration, controls, and extensible modules.'
        })

class AIAgentOrchestrationLandingView(View):
    def get(self, request):
        return render(request, 'pages/ai_agent_orchestration_landing.html', {
            'title': 'AAPPLIFY | AI Agent Orchestration',
            'meta_title': 'AAPPLIFY | AI Agent Orchestration Control Plane',
            'meta_description': 'Operate AI agents with real-time monitoring, governance, identity management, and security from a unified control plane.'
        })
...
class AboutView(View):
    def get(self, request):
        return render(request, 'pages/about.html', {
            'title': 'Why AAPPLIFY',
            'meta_title': 'AAPPLIFY | Why We Built an Enterprise Control Plane',
            'meta_description': 'Learn why AAPPLIFY was built to turn identity, change, data, security, and policy context into governed action for enterprise teams.',
        })
...
class FAQView(View):
    def get(self, request):
        return render(request, 'pages/faq.html', {
            'title': 'Control Plane FAQ',
            'meta_title': 'AAPPLIFY | Enterprise Control Plane FAQ',
            'meta_description': 'Answers to common questions about AAPPLIFY consoles, capabilities, signals, orchestration, controls, policy files, pilots, and integrations.',
        })
